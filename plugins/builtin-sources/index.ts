...
const SourceFetchOutputSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    download: z.custom<(destinationPath: string) => Promise<void>>(
      (value) => typeof value === "function",
      "Expected a download function",
    ),
  }),
]);

export const AvailableSourcesApi = definePluginApiSchema(
  "AvailableSources",
  z.undefined(),
  z.array(SourceDescriptorSchema),
);

export const SourceFetchApi = definePluginApiSchema(
  "SourceFetch",
...
function init(initContext: PluginInitContext): Promise<void> | void {
    ...
    return [
      {
        ...
        description:
          "A source that allows you to provide a direct URL to a file on a server.",
        type: "stream",
        needs: ["direct-url"],
      },
      {
  ...
  api.provideSchema(SourceFetchApi, async (input) => {
    ...
    if (sourceId === "direct-url") {
      ...
      return {
        ...
        download: async (destinationPath: string) => {
  ...
  void (async () => {
    ...
    const [fileSystemEventProvider] = await api.call(
      queryProvider.id,
      "QueryPlugins",
      {
    ...
    events.on(fileSystemEventProvider.id, async (event) => {
      logger.log(
        `received provider event ${event.type} from ${event.providerId}: ${JSON.stringify(event.payload)}`,
      );
    });

    await events.emit(
      "sources.ready",
      SourcesReadyEventPayloadSchema.parse({ pluginId }),
    );
