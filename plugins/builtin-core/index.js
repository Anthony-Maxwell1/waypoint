...
const queryPluginsInputSchema = z
    ...
    .optional();
export const QueryPluginsApi = definePluginApiSchema("QueryPlugins", queryPluginsInputSchema, z.array(manifestEntrySchema));
export const PluginByIdApi = definePluginApiSchema("PluginById", z.string(), manifestEntrySchema.nullable());
...
function init(initContext) {
    ...
    api.provideSchema(QueryPluginsApi, async (filters) => {
        ...
        return manifest.all().filter((plugin) => {
            ...
            const providesMatches = provides.length === 0 ||
    ...
    void events.emit("core.ready", CoreReadyEventPayloadSchema.parse({ pluginId }));
