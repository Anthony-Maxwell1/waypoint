...
import { defaultFlowDocument, flowDocumentSchema, } from "./flow-config.js";
...
function createEventBus(pluginId) {
    return {
        ...
        on(providerPluginId, handler) {
            ...
            if (!provider) {
                throw new Error(`Cannot subscribe to any provider '${providerPluginId}'`);
            }
            if (!provider.apis.provides.includes("EventProvider")) {
                throw new Error(`Plugin '${providerPluginId}' does not provide EventProvider`);
            }
...
function createApiRegistry(pluginId) {
    const provide = (name, handler) => {
        ...
        if (providers.has(pluginId)) {
            throw new Error(`API '${name}' is already provided by '${pluginId}' and cannot be redefined by the same provider`);
        }
        providers.set(pluginId, handler);
    };
    const call = async (providerPluginId, name, input) => {
        ...
        if (!handler) {
            throw new Error(`API '${name}' is not provided by plugin '${providerPluginId}'`);
        }
    ...
    const provideSchema = (schema, handler) => {
    ...
    const callSchema = async (providerPluginId, schema, input) => {
...
function getValueByPath(value, pathExpression) {
...
function resolveProviderSelector(selector, vars) {
...
async function findQueryProviderId(api) {
...
async function queryProvidersByCapabilities(api, requirements) {
    ...
    return candidates.filter((candidate) => {
        ...
        const anyOk = anyOf.length === 0 ||
            anyOf.some((capability) => provided.has(capability));
        const noneOk = noneOf.length === 0 ||
        ...
        switch (step.type) {
            case "query": {
                const matches = await queryProvidersByCapabilities(env.api, {
                    allOf: Array.isArray(step.allOf) ? step.allOf : [],
                    anyOf: Array.isArray(step.anyOf) ? step.anyOf : [],
                    noneOf: Array.isArray(step.noneOf) ? step.noneOf : [],
                });
                env.vars[step.assign] = matches.map((plugin) => plugin.id);
                break;
            }
            case "set": {
                env.vars[step.assign] = resolveTemplate(step.value, env.vars);
                break;
            }
            case "call": {
            ...
            case "if": {
                ...
                const providerId = resolveProviderSelector(condition.provider, env.vars);
                const input = resolveTemplate(condition.input, env.vars);
                const outcome = await env.api.call(providerId, String(condition.api), input);
...
function saveFlowDocument(document) {
    ensureDataDirectory();
    writeFileSync(FLOWS_FILE, `${JSON.stringify(document, null, 2)}\n`, "utf-8");
}
async function runStreamLikeEndpoint(endpoint, input, flowDocument) {
...
async function runReserveEndpoint(input, flowDocument) {
...
async function runSearchEndpoint(input, flowDocument) {
    const searchConfig = flowDocument.search;
    const candidates = new Map();
    async function queryAndSearch(capabilities, apiName) {
        const providers = await queryProvidersByCapabilities(createApiRegistry("waypoint-runtime"), {
            allOf: capabilities,
        });
        for (const provider of providers) {
            const results = await createApiRegistry("waypoint-runtime").call(provider.id, apiName, {
                query: input.query,
                filters: input.filters ?? {},
            });
...
function cleanupPluginState(pluginId) {
...
async function loadPlugin(plugin) {
    try {
        ...
        loadedPlugins.push({ manifest: plugin, module: mod });
        return true;
    }
    catch (e) {
...
async function loadAllPlugins(manifest) {
    ...
    while (remaining.length > 0) {
        ...
        if (readyThisPass.length === 0) {
            ...
            for (const plugin of stillWaiting) {
                const missingDeps = plugin.dependencies.onLoad.filter((d) => !loadedIds.has(d));
                const missingApis = plugin.apis.onLoad.filter((a) => !providedApis.has(a));
                console.error(`  '${plugin.id}' waiting on: deps=[${missingDeps}] apis=[${missingApis}]`);
            }
            process.exit(1);
        }
...
function renderUi(flowDocument) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function sendStream(response, value) {
    const streamValue = isPlainObject(value) && "stream" in value
        ? value.stream
        : undefined;
    if (!streamValue) {
    ...
    if (typeof streamValue.getReader === "function") {
        Readable.fromWeb(streamValue).pipe(response);
        return;
    }
...
async function executeEndpoint(endpoint, body) {
    ...
    if (endpoint === "search") {
        ...
        async function collect(capabilities, apiName) {
        ...
        if (Object.keys(filters).length > 0) {
            await collect(flowDocument.search.filterCapabilities, "SearchWithFilters");
    ...
    const vars = {
        title: isPlainObject(body) && typeof body.title === "string"
            ? body.title
            : "Untitled",
        filters: isPlainObject(body) && isPlainObject(body.filters) ? body.filters : {},
    };
    const result = await runSteps(flowDocument[endpoint], {
        api: createApiRegistry("waypoint-runtime"),
        manifest: manifestView,
...
const server = createServer(async (request, response) => {
    ...
    try {
        ...
        if (request.method === "POST" &&
            ["/api/stream", "/api/download", "/api/reserve", "/api/search"].includes(url)) {
